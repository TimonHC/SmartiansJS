...
myMines = [25, 28, 55, 58];
enemyMines = [11,21, 39, 69, 96];
mockableAttackCoords = [1, 3, 4];
aiDefaultAttackCoords = [3, 4, 7, 8, 13, 14, 17, 18, 21, 22, //дефолтные координаты по которым максимум за 24 хода найдется четырехпалубный
...
aiLastSuccessAttackCoords = 0; //память, чтоб держать прошлое попаданиеё
...
function fillField(field) { //заполняет игровое поле "@"
    for (let i = 0; i < 100; i++) {
        field.push('@');
    }
    return field;
...
function putShipsOnField(field, ships){ //добавляет корабли на поле
    for (let i = 0; i <= ships.length - 1; i++) {
        field[ships[i]-1] = '#';
    }
}
function attack(field, coords) { //функция атаки
    switch (field[coords-1]){
        ...
        case '@' :  field[coords-1] = '*';
                break;
        case '*' : console.log('Заведомо известная ячейка'); break;
        case 'X' : console.log('Заведомо известная ячейка'); break;
        case '+' : shipExplodesOnMine(field);
        break;
        default :  console.log('неверные координаты'); break;
    }
}
function printField(field){ //функция вывода поля в консоль
    switch(field) {
        case myField : console.log('Моё поле: '); break;
        case guessField : console.log('Враже поле: '); break;
        default : console.log('Неверное поле...'); break;
    }
    fieldCoords = [ //координаты (А-J), (01-10)
        ['   |A_B_C_D_E_F_G_H_I_J|',],
...
function repaintFields(){ //функция вывода обоих полей в консоль
    printField(myField);
...
/*
    ...
    for (let j = 0; j < 1000; j++) {
        mockableAttackCoords[j] = getRandomIntInclusive(1, 100);
    }
*/
